...
export class AdminController{
  ...
  async updateCustomer(@Param('id') id:string,@Body() b:any){
    ...
    vals.push(id);const q=await this.db.pool.query('UPDATE "User" SET '+fields.join(',')+',"updatedAt"=NOW() WHERE id=
...
}+vals.length+' AND role=\'customer\' RETURNING id,name,username,email,status,balance',vals);
    if(!q.rowCount)throw new NotFoundException('Customer not found');return q.rows[0];
  }
