...
export class AdminController {
  ...
  async customers(@Query('status') status?:string,@Query('limit') limit?:string,@Query('offset') offset?:string) {
    ...
    const q=await this.db.pool.query(
      'SELECT id,name,username,email,role,status,balance,"createdAt" FROM "User" '+where+
  ...
  async createCustomer(@Body() b:any) {
    ...
    try {
      ...
      const q=await client.query(
        'INSERT INTO "User" (name,username,email,"passwordHash",role,status,balance,"createdAt","updatedAt") VALUES ($1,$2,$3,$4,\'customer\',\'active\',0,NOW(),NOW()) RETURNING id,name,username,email,status,balance',
  ...
  async updateCustomer(@Param('id') id:string,@Body() b:any) {
    ...
    if(b.status!==undefined){
      if(!['active','suspended','pending','disabled'].includes(String(b.status))) throw new BadRequestException('Invalid status');
      fields.push('status=$'+(vals.length+1));vals.push(String(b.status));
    }
    if(b.password!==undefined){
      if(String(b.password).length<8) throw new BadRequestException('Password must be at least 8 characters');
      fields.push('"passwordHash"=$'+(vals.length+1));vals.push(hashPassword(String(b.password)));
    }
    if(!fields.length) throw new BadRequestException('No changes supplied');
    vals.push(id);
    const q=await this.db.pool.query(
      'UPDATE "User" SET '+fields.join(',')+',"updatedAt"=NOW() WHERE id=$'+vals.length+' AND role=\'customer\' RETURNING id,name,username,email,status,balance',
      vals
    );
    if(!q.rowCount) throw new NotFoundException('Customer not found');
    return q.rows[0];
  }
  ...
  async deposits(@Query('status') status?:string) {
    ...
    const q=await this.db.pool.query(
      'SELECT d.id,d."userId",u.username,u.name,d.amount,d.status,d.reference,d.provider,d.metadata,d."createdAt" FROM "Deposit" d JOIN "User" u ON u.id=d."userId" '+where+' ORDER BY d.id DESC LIMIT 200',
      params
    );
    return q.rows;
  }
  ...
  async approveDeposit(@Req() req:any,@Param('id') id:string) {
    ...
    try {
      ...
      const amount=Number(d.rows[0].amount);
      const after=(before+amount).toFixed(6);
      ...
      await client.query(
        'INSERT INTO "Transaction" ("userId",amount,direction,"balanceBefore","balanceAfter",description,"createdAt") VALUES ($1,$2,\'credit\',$3,$4,$5,NOW())',
        [d.rows[0].userId,d.rows[0].amount,before.toFixed(6),after,'Deposit approved '+(d.rows[0].reference||txRef)]
      );
      await client.query(
        'UPDATE "Deposit" SET status=\'approved\',metadata=COALESCE(metadata,\'{}\'::jsonb)||$1::jsonb WHERE id=$2',
        [JSON.stringify({approvedBy:String(req.userId),approvedAt:new Date().toISOString()}),id]
      );
      await client.query('COMMIT');
      return {status:'approved',depositId:id,balanceAfter:after};
    } catch(e) {
      await client.query('ROLLBACK');
      throw e;
    } finally { client.release(); }
  ...
  async rejectDeposit(@Req() req:any,@Param('id') id:string,@Body() b:any) {
    ...
    const q=await this.db.pool.query(
      'UPDATE "Deposit" SET status=\'rejected\',metadata=COALESCE(metadata,\'{}\'::jsonb)||$1::jsonb WHERE id=$2 AND status=\'pending\' RETURNING id,status',
      [JSON.stringify({rejectedBy:String(req.userId),reason,rejectedAt:new Date().toISOString()}),id]
    );
