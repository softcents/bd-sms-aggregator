 ...
 async send(dto:SendSmsDto,userIdHeader?:string,idempotencyKey?:string){
  ...
  await this.db.tx(async c=>{
    ...
    const rateRows=(await c.query(`SELECT DISTINCT ON (tr."operatorId") tr."operatorId",tr."maskingPrice",tr."nonMaskingPrice"
      ...
      ORDER BY tr."operatorId",a."effectiveFrom" DESC,tr."effectiveFrom" DESC`,[userId.toString()])).rows;
    ...
    const batch=(await c.query('INSERT INTO "Batch" ("externalId","userId","senderId",body,status,"totalRecipients","validCount","createdAt","updatedAt") VALUES ($1,$2,$3,$4,\'planning\',$5,$5,NOW(),NOW()) RETURNING id',[batchExternal,userId.toString(),sender.id,dto.body,staged.length])).rows[0];
