...
export class CampaignService{
 ...
 async control(uid:string,id:string,action:string){
   ...
   else if(action==='resume'){if(batch.status!=='paused')throw new BadRequestException('Campaign is not paused');await c.query('UPDATE "Batch" SET status=\'processing\',"pausedAt"=NULL,"updatedAt"=NOW() WHERE id=$1',[batch.id]);await this.queue.publishCampaign(String(batch.id));}
   else if(action==='cancel'){if(['completed','cancelled'].includes(batch.status))throw new BadRequestException('Campaign already finished');const u=await c.query('SELECT balance FROM "User" WHERE id=$1 FOR UPDATE',[uid]);
   const mq=await c.query('SELECT COALESCE(SUM(cost),0)::numeric refund FROM "Message" WHERE "batchId"=$1 AND status IN (\'queued\',\'pending\')',[batch.id]);
   const cq=await c.query('SELECT COALESCE(SUM(cost),0)::numeric refund FROM "CampaignRecipient" WHERE "batchId"=$1 AND status=\'pending\'',[batch.id]);
   const refund=new Decimal(String(mq.rows[0].refund||'0')).add(String(cq.rows[0].refund||'0'));
   const before=String(u.rows[0].balance);const after=new Decimal(before).add(refund).toFixed(6);
   await c.query('UPDATE "Message" SET status=\'failed\',"failedReason"=\'campaign_cancelled\',"failedAt"=NOW(),"refundReference"=\'campaign-refund:\'||"externalId","updatedAt"=NOW() WHERE "batchId"=$1 AND status IN (\'queued\',\'pending\')',[batch.id]);
   await c.query('UPDATE "CampaignRecipient" SET status=\'cancelled\',"updatedAt"=NOW() WHERE "batchId"=$1 AND status=\'pending\'',[batch.id]);
   if(refund.gt(0)){await c.query('UPDATE "User" SET balance=$1,"updatedAt"=NOW() WHERE id=$2',[after,uid]);await c.query('INSERT INTO "Transaction" ("userId",amount,direction,"balanceBefore","balanceAfter",description,"createdAt") VALUES ($1,$2,\'credit\',$3,$4,$5,NOW())',[uid,refund.toFixed(6),before,after,'Campaign cancellation '+id]);}
   await c.query('UPDATE "Batch" SET status=\'cancelled\',"cancelledAt"=NOW(),"updatedAt"=NOW() WHERE id=$1',[batch.id]);}
