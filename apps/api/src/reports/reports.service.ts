...
export class ReportsService {
  ...
  async summary(userId:string){
    const q=await this.db.pool.query(`SELECT
      COUNT(*)::int total,
      ...
      COALESCE(SUM(cost),0)::numeric total_cost
      FROM "Message" WHERE "userId"=$1`,[userId]);
  ...
  async batch(userId:string,id:string){
    const q=await this.db.pool.query(`SELECT b.id,b."externalId",b.status,b."totalRecipients",b."validCount",b."invalidCount",
      ...
      COALESCE(SUM(m.cost),0)::numeric cost
      FROM "Batch" b LEFT JOIN "Message" m ON m."batchId"=b.id
      WHERE b."userId"=$1 AND (b.id::text=$2 OR b."externalId"=$2)
      GROUP BY b.id`,[userId,id]);
  ...
  async messages(userId:string,limit=50,offset=0,status?:string){
    ...
    const countQ=await this.db.pool.query(`SELECT COUNT(*)::int total FROM "Message" ${where}`,params);
    params.push(lim,off);
    const q=await this.db.pool.query(
      `SELECT "externalId",to,status,cost,parts,"gatewayMessageId","sentAt","deliveredAt","failedReason","createdAt"
       FROM "Message" ${where} ORDER BY "createdAt" DESC LIMIT $${params.length-1} OFFSET $${params.length}`,params);
    return {data:q.rows,limit:lim,offset:off,total:countQ.rows[0].total};
  }
}
