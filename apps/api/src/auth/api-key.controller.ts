...
export class ApiKeyController {
  ...
  @Get() async list(@Req() req:any){
    const q=await this.db.pool.query('SELECT id,"externalId",name,"lastUsedAt","lastIp","expiresAt",enabled,"allowedIps","createdAt" FROM "ApiKey" WHERE "userId"=$1 ORDER BY "createdAt" DESC',[this.uid(req)]);
    return q.rows;
  }
  @Post() async create(@Req() req:any,@Body() body:any){
    ...
    await this.db.pool.query('INSERT INTO "ApiKey" ("externalId","userId",name,"keyHash",enabled,"allowedIps","expiresAt","createdAt") VALUES ($1,$2,$3,$4,true,$5,$6,NOW())',[externalId,this.uid(req),name,hash,ips,expires]);
  ...
  @Patch(':id') async update(@Req() req:any,@Param('id') id:string,@Body() body:any){
    const ips=body.allowedIps===undefined?null:this.normalizeIps(body.allowedIps);
