...
function App(){
  ...
  async function send(e:any){e.preventDefault();setMsg('Sending...');
    const r=await fetch(API+'/sms/send',{method:'POST',headers:authHeaders(),body:JSON.stringify({senderId:sender,to:to.split(/[,\s]+/).filter(Boolean),body,isUnicode:/[^\x00-\x7F]/.test(body)})});
    const d=await r.json();setMsg(r.ok?'SMS accepted: '+d.recipients+' recipients':(d.message||'Send failed'));if(r.ok){setTo('');setBody('');load()}
  }
  async function createKey(e:any){e.preventDefault();const r=await fetch(API+'/api-keys',{method:'POST',headers:authHeaders(),body:JSON.stringify({name:keyName})});const d=await r.json();if(r.ok){setNewKey(d.apiKey);setKeyName('');load()}else setMsg(d.message||'API key creation failed')}
  async function createSender(e:any){e.preventDefault();const r=await fetch(API+'/senders',{method:'POST',headers:authHeaders(),body:JSON.stringify(sf)});const d=await r.json();if(r.ok){setSf({senderId:'',type:'non-masking',billMsisdn:''});load()}else setMsg(d.message||'Sender creation failed')}
  async function toggleSender(id:string,status:string){await fetch(API+'/senders/'+id,{method:'PATCH',headers:authHeaders(),body:JSON.stringify({status:status==='active'?'inactive':'active'})});load()}
  async function revoke(id:string){await fetch(API+'/api-keys/'+id,{method:'DELETE',headers:authHeaders()});load()}
  async function requestDeposit(e:any){e.preventDefault();const r=await fetch(API+'/billing/deposits',{method:'POST',headers:authHeaders(),body:JSON.stringify({amount:deposit,reference})});const d=await r.json();setMsg(r.ok?'Deposit request submitted':'Deposit request failed');if(r.ok){setDeposit('');setReference('');load()}}
  async function createCampaign(e:any){e.preventDefault();setMsg('Creating campaign...');const r=await fetch(API+'/campaigns',{method:'POST',headers:authHeaders(),body:JSON.stringify({senderId:campaign.senderId,body:campaign.body,recipients:campaign.recipients.split(/[,\s]+/).filter(Boolean),groupIds:campaign.groupIds,scheduledAt:campaign.scheduledAt||undefined})});const d=await r.json();setMsg(r.ok?'Campaign accepted: '+d.recipients+' recipients':(d.message||'Campaign failed'));if(r.ok){setCampaign({...campaign,body:'',recipients:''});load()}}
  async function campaignAction(id:string,action:string){await fetch(API+'/campaigns/'+id+'/'+action,{method:'POST',headers:authHeaders()});load()}
  async function importContacts(e:any){e.preventDefault();const r=await fetch(API+'/campaigns/contacts/import',{method:'POST',headers:authHeaders(),body:JSON.stringify({fileName:'contacts.csv',csv:importCsv})});const d=await r.json();setMsg(r.ok?'Import queued: '+d.importId:(d.message||'Import failed'));if(r.ok){setImportCsv('');load()}}
